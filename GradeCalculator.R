...
data <- read.csv("~/Documents/#Programming/GradeCalculatorAPI/GradeCalculator/GradeRubric.csv")

key <- GradeRubric[,1]
return <- GradeRubric[,4]
points <- GradeRubric[,3]

GradeRubric %>%
  filter(pointsValue == points) %>%
  vlookup(ref = key, column = return, range = F)


#### White Board

if(F){
  gradeRubric_df <- read.csv("~/Documents/#Programming/GradeCalculatorAPI/GradeCalculator/GradeRubric.csv")
